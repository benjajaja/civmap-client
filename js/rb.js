...
var getSoil = (soil_material, soil_max_layers, soil_layer_offset, soil_bonus_per_layer) => soil_material
        ? soil_material.replace('_', ' ')
          + ' ×' + soil_max_layers
          + (soil_layer_offset ? '+' + soil_layer_offset : '')
          + ' = ' + ((soil_bonus_per_layer * soil_max_layers + 1) * 100) + '%'
        : null;

var displayGrowth = (value, persistent_growth_period) => !value ? null : (persistent_growth_period ? Math.round((persistent_growth_period / value) * 100) / 100 + 'h' : value * 100 + '%');
...
var calculateIdeals = (growth, bestFactor) => {
  ...
  if (growth.persistent_growth_period) {
    ...
    return Math.round((growth.persistent_growth_period / factor) * 100) / 100 + 'h';
  } else {
    return growth.base_rate * bestFactor * 100 + '%';
  }
};

var ideals = (key, growth, bestFactor) => {
  var hasDesc = typeof idealsMap[key] !== 'undefined';
  return [calculateIdeals(growth, bestFactor)
    + (hasDesc ? ' - ' + idealsMap[key]: ''),
    ['class', 'success']];
};

var biomeCell = (growth, bestValue, value) => {
  if (value === bestValue) {
...
var biomeGrowth = (growth, biomesAliasMap, biome) => {
  ...
  if (typeof growth.biomes[biome] !== 'undefined') {
    ...
    return growth.biomes[biomesAliasMap[biome].find((alias) => {
...
xhr.onload = (e) => {
  ...
  var biomes = _.uniq(_.flatten(_.values(config.biome_aliases))).filter((biome) => ['ICE_MOUNTAINS', 'TAIGA_HILLS', 'FOREST_HILLS', 'SMALL_MOUNTAINS', 'DESERT_HILLS', 'JUNGLE_HILLS', 'MUSHROOM_SHORE', 'SKY'].indexOf(biome) === -1);
  biomes.push('SKY');
  var biomesAliasMap = _.reduce(config.biome_aliases, (map, biomes, alias) => {
    return _.reduce(biomes, (map, biome) => {
      if (map[biome]) {
  ...
  row(thead, [null,
    // span(null, 'Base growth', 'glyphicon glyphicon-download'),
    ...
    span(null, 'Greenhouse: When placed immediately adjacent to glowstone (sharing a face with the block) a crop will ignore the sunlight modifiers and use this one instead, if this rate is an improvement. The base rate and soil modifiers still apply, biome modifier applies depending on "Greenhouse ignores biome".', 'glyphicon glyphicon-home'),
    span(null, 'Greenhouse ignores biome requirements? If yes and greenhouse, then biome rate is ignored.', 'glyphicon glyphicon-globe'),
    span(null, 'When placed below the crop, blocks of this type provide a bonus, up to the amount listed: material ×max-layers +offset-below-from-crop = growth', 'glyphicon glyphicon-tower')
  ]
  .concat(biomes.map((biome) => (biomeNamesMap[biome] || biome)))
  ...
  _.each(config.growth, (growth, key) => {
    ...
    row(tbody, [key,
      // !growth.persistent_growth_period ?  : ['✗', ['class','info']],
    ...
    .concat([ideals(key, growth, bestValue)]));
  });
  ...
  row(tbody, ['RAW_FISH',
    ...
    .concat([ideals('RAW_FISH', config.fish_drops.mat_RAW_FISH, bestValue)]));
